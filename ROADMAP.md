...
- **v0.3.5** (2026-05-23): hotfix — drop the Phase-1 `AVAILABLE_MODULES` allowlist on `/notes/`. v0.3.4 shipped notes for all 11 modules but the index was still rendering 8 of them as "Coming soon" because the page hardcoded a three-module allowlist that was never updated.
- **v0.3.6** (2026-05-23): rename four modules to field-standard names (closes #46) — `Public Systems Thinking` → `Systems thinking for policy`; `Public Economics — Incentives` → `Public economics (incentives)`; `Public Economics — Markets` → `Public economics (markets)`; `Microeconomics — Demand & Supply` → `Microeconomics (demand & supply)`. Surfaced while drafting the alumni-share post — old names read as Takshashila-internal coinage to non-alumni readers.
- **v0.3.7** (2026-05-23): drop GCPP module codes (`CP 10`, `CG 1`, etc.) from the public UI — picker, notes index, per-module header. Same logic as v0.3.6 renames; codes are Takshashila syllabus numbering and don't help non-Takshashila readers. Codes remain in `modules.json` data for internal cross-reference.
- **v0.3.8** (2026-05-23): shorter, direct names for the three econ modules — `Public economics (incentives)` → `Incentives`; `Public economics (markets)` → `Markets`; `Microeconomics (demand & supply)` → `Demand & Supply`. v0.3.6 kept the "Public economics" umbrella for clarity but the content is closer to microeconomic foundations + market processes (Hayek-style) than the formal public-economics canon — the umbrella was adding jargon without clarity.
