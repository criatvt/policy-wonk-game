...
Slug coverage gap to be aware of: not every `topic` slug used in `src/data/questions/<module-id>.json` has a 1:1 note file. The end-screen "Browse notes for [topic]" link in [`EndScreen.jsx`](src/components/game/EndScreen.jsx) currently 404s when a player falls on a question whose topic slug doesn't yet exist as a note. Expanding coverage to all question-bank slugs (or adding a graceful fallback page) is open.
