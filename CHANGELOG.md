...
- **Email provider swap — sign-in is dead in production.** Resend's free tier verifies exactly **one** sending domain per account, and that slot is held by an unrelated project, so `policywonkgame.aasifj.com` could never be verified there: every magic-link send is rejected and `/api/auth/send-link` fails silently behind its deliberately generic 200. Cloudflare Email Sending was evaluated and rejected — it requires the Workers Paid plan. **Brevo** is the chosen replacement (free tier allows multiple verified sender domains at 300 emails/day). The code change is staged locally but deliberately **not committed**: it renames `RESEND_API_KEY` → `BREVO_API_KEY` and `RESEND_FROM` → `EMAIL_FROM`, so shipping it before the Brevo account exists and the secrets are set would turn a silent failure into a visible 500. Land it together with: (1) Brevo account + domain verification (DNS is on Cloudflare), (2) `wrangler pages secret put BREVO_API_KEY` for **both** preview and production, (3) an end-to-end signup smoke test on both.
