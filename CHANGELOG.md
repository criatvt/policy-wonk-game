...
- **`/play` was blank in production for every visitor, guest or signed in (regression in v0.1.4).** The milestone-confetti effect added to `GameContainer` read `state.status` and `state.currentRung` in its dependency array. A dependency array is evaluated as an argument to `useEffect`, which means it runs during render — and `state` is `null` on every screen before a game starts (onboarding, rules, module pick), so the very first render of the island threw `TypeError: Cannot read properties of null (reading 'status')`. With no error boundary and `client:only="react"` (so no server-rendered fallback either), React unmounted the whole root: the Astro-rendered header and footer still painted and the game area was empty, which made it look like a sign-in problem rather than a crash. The deps are now optional-chained and the body takes a `!state` guard. Milestone confetti is otherwise unchanged.

## [0.1.4] — 2026-08-24 — Align the web build with the iOS app
